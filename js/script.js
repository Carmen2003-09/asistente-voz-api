...
const API_URL = "http:localhost:8080/api-gpt-php/endpoints/chat.php";
  ...
  const guardarPalabraClaveIot = async (palabraClave) => {
    try {
      await fetch("44.211.50.100/iot-api-php/controllers/AddIotDevice.php", {
        ...
        body: JSON.stringify({ status: palabraClave })
      });
    } catch (error) {
      console.error('Error al guardar la palabra clave en IOT:', error);
    }
  };
...
console.log("Texto reconocido:", textoNormalizado);
      ...
      if (textoNormalizado.includes(ordenPrefijo + " SALIR")) {
        ...
        if (token) {  
          ...
          // Guardar la palabra clave en la URL de IOT
          guardarPalabraClaveIot(token);
